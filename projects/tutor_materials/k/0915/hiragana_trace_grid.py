...
CONTENT_FILE_PATH = "hiragana_data/hira_wordsearch_mix_01.json"  # このPythonファイル自身を基準とした文字配置JSONの相対パス。
OUTPUT_FILE_PREFIX = "grid"  # 出力画像名の先頭につける文字列。hiragana.jsonならgrid_hiragana.pngになる。
...
class HiraganaTraceGrid(Scene):
    """外部JSONで全マスを自由指定できる練習シート."""
